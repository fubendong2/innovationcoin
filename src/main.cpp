...
uint256 hashGenesisBlock("0x298643867144aca005fec38f9882730a5c736ac0b674dd27e94f8d3976a74d1b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Innovationcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Innovationcoin Signed Message:\n";
...
static const int64 nTargetTimespan = 60 * 60;	// Innovationcoin: 1 hour
static const int64 nTargetSpacing = 60;						// Innovationcoin: 30 sec
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 60 * 60; // 60 Minutes
static const int64 nTargetSpacingRe = 30; // Innovationcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
	...
	// Innovationcoin: This fixes an issue where a 51% attack can change difficulty at will.
		...
		const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
	...
	// fMiner is true when called from the internal innovationcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
	...
	if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
	{
		...
		printf("*** %s\n", strMessage.c_str());
		uiInterface.ThreadSafeMessageBox(strMessage, "Innovationcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
		StartShutdown();
		return false;
	}
	return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
	if (fTestNet)
	{
		...
		hashGenesisBlock = uint256("0x298643867144aca005fec38f9882730a5c736ac0b674dd27e94f8d3976a74d1b");
	}
	...
	if (mapBlockIndex.empty())
	{
		...
		// Genesis block
		const char* pszTimestamp = "Innovation  coin";
		CTransaction txNew;
		txNew.vin.resize(1);
		txNew.vout.resize(1);
		...
		assert(block.hashMerkleRoot == uint256("0xaeab996b97f37fa9609b51fdca5a8bd44009e8c2a2f91dff7913b3d13266951a"));
...
{
	return false;
	//commenting out this code until we properly implement IVC alerts.
	/*
	   if (!CheckSignature())
	   return false;
	   if (!IsInEffect())
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Innovationcoin: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
	...
	int64 nFees = 0;
	{
		...
		while (!mapPriority.empty())
		{
			...
			// Innovationcoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
